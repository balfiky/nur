...
def test_detect_learning_request_requires_explicit_learning_intent():
    ...
    with pytest.raises(LearningIntakeError, match="URL or a longer pasted text"):
        detect_learning_request("I have some text and I want you to ingest and learn")
...
def test_pending_learning_intake_accepts_next_long_paste(tmp_path):
    async def run():
        config = _config(tmp_path)
        manager = SessionManager(config)
        session = SimpleNamespace(turn_index=1, pending_learning_intake=None)
        try:
            note = await manager._maybe_run_learning_intake(
                session,
                "alice",
...
def test_pending_learning_intake_ignores_bare_or_expired_paste(tmp_path):
    async def run():
        config = _config(tmp_path)
        manager = SessionManager(config)
        paste = (
